...
class MergeSchemaParquet:

    def __init__(self, spark, sc):
        self.spark = spark
        self.sc = sc

    def is_schema_equal(self, path1, path2):
        """
        Input:
        ...
        df1 = self.spark.read.parquet(path1).limit(0)
        df2 = self.spark.read.parquet(path2).limit(0)
        
        return df1.schema == df2.schema

    def identify_schema_changes(self, path, partition_list = []):
        """
        Input:
        ...
        for dir in sorted([d for d in os.listdir(path) if d.find("=") != -1]) if partition_list == [] else partition_list:
            ...
            elif not self.is_schema_equal(dict[idx]["init_path"], path + "/" + dir):
                idx = idx + 1
                dict[idx] = {"init_path": path + "/" + dir, "final_path": path + "/" + dir}
    ...
    def merge_schemas_incremental(self, dir_path, ctrl_file):
        """
        Input: 
        - dir_path: Entity path containing partitions.
        ...
        idx = 0
        last_modified_ts_ctrl = merge_schema_helper.get_last_read_ts(ctrl_file)
        ...
        if merge_schema_helper.get_modified_partitions(dir_path, last_modified_ts_ctrl) != []:
            ...
            try:
                if len(self.identify_schema_changes(dir_path, lst_dir).keys()) > 1:
                    print("Different schemas identified:")
                    print(json.dumps(self.identify_schema_changes(dir_path, lst_dir), indent = 4))
                ...
                for dir in lst_dir:
                    ...
                    # Get schema
                    schema = self.spark.read.parquet(dir_path + "/" + dir).limit(0).schema

                    # Read parquet file converting to string
    ...
    def merge_schemas_full(self, dir_path):
        """
        Input: 
        - dir_path: Entity path containing partitions.
